...
<!-- 
    IZ customization: specific link to display document in swisscovery
...
<xsl:stylesheet version="1.0"
        ...
        <xsl:choose>
            <xsl:when test=" $lang = 'de' ">
                <a>
                    ...
                    https://hsg.swisscovery.ch/permalink/41SLSP_HSG/1o0fmot/alma<xsl:value-of
                            select="notification_data/mms_id" /></xsl:attribute>
		<xsl:attribute
                        name="target">_blank</xsl:attribute>
                    https://hsg.swisscovery.ch/permalink/41SLSP_HSG/1o0fmot/alma<xsl:value-of
                        select="notification_data/mms_id" /></a>
            </xsl:when>
            <xsl:otherwise>
                <a>
                    <xsl:attribute name="href">
                    https://hsg.swisscovery.ch/permalink/41SLSP_HSG/1dp04e2/alma<xsl:value-of
                            select="notification_data/mms_id" /></xsl:attribute>
		<xsl:attribute
                        name="target">_blank</xsl:attribute>
                    https://hsg.swisscovery.ch/permalink/41SLSP_HSG/1dp04e2/alma<xsl:value-of
    ...
    <!-- Prints the IZ message stored in label 'department' for the language of the letter.
		...
		Usage:
			...
			2. <<already done by SLSP in this letter>>Insert the template: <xsl:call-template
    name="SLSP-IZMessage"/> -->
    <xsl:template name="SLSP-IZMessage">
        ...
        <xsl:if test="$notice != '' and $notice != 'blank'">
            ...
            :</strong>&#160;<xsl:value-of select="$notice" disable-output-escaping="yes" />
        </xsl:if>
    </xsl:template>
        ...
        <html>
            ...
            <body>
                ...
                <table role='presentation' cellspacing="0" cellpadding="5" border="0">
                    ...
                    <tr>
                        <td> @@orderNumber@@:&#160;<xsl:value-of
                                select="notification_data/line_number" />
						<br />
                        </td>
                    </tr>
                    ...
                    <xsl:if test="contains(notification_data/letter_params/subject, 'activated') or contains(notification_data/letter_params/subject, 'aktiviert') or contains(notification_data/letter_params/subject, 'activé') or contains(notification_data/letter_params/subject, 'attivato')">
                        <tr>
                            <td>
                                <br />
                                @@mmsId@@: <xsl:call-template name="displayLink" />
                            </td>
                        </tr>
                    </xsl:if>
                    <tr>
